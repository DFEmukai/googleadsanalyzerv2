...
## 有効キャンペーンのみ分析
- 分析対象は有効（ENABLED/ACTIVE）なキャンペーンのみです
- 停止中（PAUSED）または削除済み（REMOVED）のキャンペーンには改善提案を生成しないでください
- データにstatusフィールドがある場合、status=pausedまたはstatus=removedのキャンペーンは無視してください
"""
